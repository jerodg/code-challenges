...
function numRescueBoats(people: number[], limit: number): number {
    ...
    const countPerson = new Array(limit).fill(0);
    ...
    let minIndex = 0; // Index for the lightest person not yet accounted for
    let maxIndex = limit - 2; // Index for the heaviest person not yet accounted for
    let boats = countPerson[limit - 1]; // Number of boats needed, initially count of people with weight equal to limit
